...
def load_kb(topic: str, max_links: int = config.MAX_LINKS, max_depth: int = config.MAX_DEPTH):
...
async def aload_kb(topic: str, max_links: int = config.MAX_LINKS, max_depth: int = config.MAX_DEPTH):
    """Load knowledge base for a specific topic."""
    await get_kb(
        topic=topic,
        chunking_model=get_mistral_model(),
