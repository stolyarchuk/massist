...
def get_deepseek_model(
    model_id: str = config.DEEPSEEK_MODEL, temperature: float = config.TEMPERATURE
) -> Model:
    return DeepSeek(
        id=model_id, api_key=config.DEEPSEEK_API_KEY, temperature=temperature
    )
...
def get_google_model(
    model_id: str = config.GOOGLE_MODEL_PRI, temperature: float = config.TEMPERATURE
) -> Model:
    return Gemini(id=model_id, api_key=config.GOOGLE_API_KEY, temperature=temperature)
...
def get_vllm_embedder(
    ...
    return OpenAIEmbedder(
        id=model_id, dimensions=dims, base_url=base_url, api_key=config.VLLM_API_KEY
    )
